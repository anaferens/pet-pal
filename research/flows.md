...
**v3 — the fully state-truthful 40-step journey.** The setup walkthrough now shows the dossier *filling up*: after each section is completed the owner returns to a **progressive dossier** (`home-progress-1…6.html`, "k of 6 sections filled · Just now") where completed sections show real summaries and untouched ones stay dashed-empty. Health & jabs runs record-then-vaccine (`Health-and-jabs-record1` → `-firstrun` with both entries); Personality & care fills its six notes one by one (`-fill1…6`); Vet adds clinic then appointment (`Add-vet-clinic` → `-clinic` state → `Add-vet-record` → `-firstrun`); Emergency fills contact → vet → authorization (`Add-emergency-contact`, `-fill1`, `-fill2`, `-firstrun`). Every save passes through the shared saving screen and lands on the next truthful state.

**Success / error endings now have screens.** Every error state resolves to an existing `*-error.html` (fetch/save/link/load errors), and the happy-path endings have confirmation screens: **Share-success** (`Share-success.html` — "link ready", covers *handoff sent* / *link sent* / *dossier filled and shared*) and **Reminder-done** (`Reminder-done.html` — "marked done", covers *acted in time*). Other endings resolve to their natural destination (*nothing due* → `Whats-due-empty`, *carer understands* → `Shared-pet-view`, *has the facts* → `Emergency-allowed`, *record consulted* → `home-success`). In [ia.html](ia.html) every one of these nodes is now ✅ hover-previewable and click-through.
