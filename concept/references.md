...
- **Design techniques adopted (concrete, not impression):**
  1. **Two-surface system, no hard dividers** — a white canvas and a subtly different secondary surface; sections are separated by a *background-colour change*, never a rule/border line. (August pairs white with Warm Sand; PetPal keeps the rhythm but uses a **neutral light grey** — see the surface decision.)
...
**Warmth strategy (decided):** PetPal's warmth comes from **bold, on-point illustration + real pet photography + occasional colour blocks**, all on the clean white base — *not* from warm surfaces. Colour blocks live in the **brand / illustration layer** (à la Kinhive & Notion); the functional UI keeps its **single warm-orange action colour**, so Headspace's colour discipline still holds. This resolves the only tension: colour blocks are expression, not chrome.
...
|---|---|---|---|
| Surfaces | **white canvas `#ffffff`** · **light neutral grey `#f4f4f4` = secondary surface** (section bands, panel behind cards) · white floating cards; breaks by background change, no dividers | August Health · Airbnb | A2 |
...
| Body-text colour | **near-black** (~`#1a1a1a`), not warm brown — warmth lives in the accent, not the text | house (neutral ink) | A1 |
| Colour budget | **one action colour — warm orange `#ef7d3a`**; saturated hues only in small accents/illustrations; trust-green as a *supporting accent* (e.g., an "up-to-date" tick), not a status colour | Headspace | A4 |
...
| Warmth | **bold illustration + real photography + occasional colour blocks**, on the clean base — never warm surfaces; colour blocks stay in the brand/illustration layer, one warm-orange action colour holds | Notion · Kinhive · Airbnb · **Aboard** | A2 / A3 |
| Share screen | **explicit "who can see this" line** under the share options | Airbnb | A5 |
| Dossier | **arched colour hero (one warm colour) as the card foundation + circular pet avatar centred over the arch + name/meta**, then a **label→value list + one `Edit`** beneath | Headspace + Aboard | A1 / A3 / A2 |
...
**Surface decision — resolved to a white canvas:** the base is a **white canvas `#ffffff`** (clean and neutral — August Health, Airbnb and Headspace are all white/near-white based), with a **light neutral grey `#f4f4f4` as the secondary surface** (section bands, panel behind cards). The earlier warm near-white `#faf8f4` read too creamy, so the whole surface system is now neutral. Warmth is carried by pet **photography**, the **warm-orange accent + arched hero**, and the **serif headline** — not by a cream ground; the body ink is now a neutral near-black `#1a1a1a`. This keeps the direction faithful to August Health / Airbnb / Headspace (all near-white-based) and clears the Designer's-Taste anti-reference. See [`concept.md`](concept.md).
